...
export async function POST() {
  ...
  try {
    ...
    await new Promise<void>((resolve) => {
      browser.on('close', () => {
        console.log('🔒 Playwright browser closed by user. LinkedIn session saved.');
        resolve();
      });
    });
    ...
    return NextResponse.json({ error: formatAuthLaunchError(errorMessage(error), 'LinkedIn') }, { status: 500 });
  }
}
