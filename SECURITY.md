...
| iframe sandboxed | `allow-same-origin allow-scripts` — the provider's documented embed. **No** `allow-top-navigation`, **no** `allow-modals` | `allow-same-origin` reads as lax and is not: without it the frame is forced into an *opaque* origin and its WebSocket to the provider cannot connect, which fails silently as a permanently black rectangle. It grants no access to our page — the live view is served from the provider's origin, so same-origin policy already separates them. The combination that genuinely defeats a sandbox is `allow-same-origin` + `allow-scripts` on content served from **our own** origin, which could then rewrite its own sandbox attribute; this frame is cross-origin |
